...
class Dastan:
    ...
    def __ModifyQueueOption(self):
        option_choice = str(input(''' a) Reverse the current player queue
\n b) Swap the current player queue with the opponents queue
...
'''))
        ...
        #if option_choice == "b" or option_choice == "c" or option_choice == "d" or option_choice == "a":
        ...
        if option_choice == "b":
            if self._CurrentPlayer.SameAs(self._Players[0]):
                self._CurrentPlayer.ReplaceQueue(self._Players[1].GetPlayerQueue())
            else:
                self._CurrentPlayer.ReplaceQueue(self._Players[0].GetPlayerQueue())
            self._CurrentPlayer.ChangeScore(-3)
        ...
        if option_choice == "d":
            position = int(input("enter a choice to swap with"))
            self._CurrentPlayer.MoveItemToFront(position - 1)
            self._CurrentPlayer.ChangeScore(-3)
        if option_choice == "e":
            pass
        
            
        return self._CurrentPlayer.GetPlayerStateAsString()
        return self._CurrentPlayer.GetScore()
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                Choice = int(input("Choose move option to use from queue (1 to 3) or 9 to take the offer or 6 to modify the move option queue: "))
                if Choice == 9:
                ...
                if Choice == 6: ##edit##
                    #logic
                    modify = self.__ModifyQueueOption()
                    print(modify)
...
##edit##
    ...
    def MoveItemToFront(self, position):
        item = self.__Queue.pop(position)
...
##edit
    ...
    def ReplaceQueue(self, new_queue):
        orig_queue = self.__Queue.GetQueueAsString()
        k = 0
        for i in new_queue:
            self.__Queue.Replace(k, i)
            k += 1

        return orig_queue

    def GetPlayerQueue(self):
        return self.__Queue.PlainQueue()
