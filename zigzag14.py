...
class Dastan:
    ...
    def __WeatherEventOccurs(self):
        ...
        valid = False
        if number == 1:
            print("weather event has occured")
            while valid == False:
                
                rand_col = random.randint(1, self._NoOfColumns)
                rand_row = random.randint(1, self._NoOfRows)
                rand_square = (rand_row * 10) + rand_col

                if self._Board[self.__GetIndexOfSquare(rand_square)].GetPieceInSquare() is None and self._Board[self.__GetIndexOfSquare(rand_square)].ContainsKotla() == False:
                    valid = True

            
            self._Event.SetWeatherLocation(rand_square)
            return True
        else:
            return False
        ...
        while not GameOver:
            ...
            if MoveLegal:
                ...
                if weather_event == False:
                    ...
                    weather_event = self.__WeatherEventOccurs()
                    
                if weather_event == True:
                    square_ref = self._Event.GetWeatherLocation()
                    complete = self._Event.CountDownComplete()
                    print(square_ref)
                    print(complete)
                    if complete ==  True:
                        col = square_ref % 10
                        for Row in range(1, self._NoOfRows + 1):
                            if self._Board[(self.__GetIndexOfSquare((Row * 10) + col))].GetPieceInSquare() is not None:
                                self._Board[(self.__GetIndexOfSquare((Row * 10) + col))].RemovePiece()
                                print("yay")
                                
                            if self._Board[(self.__GetIndexOfSquare((Row * 10) + col))].ContainsKotla():
                                self._Board[(self.__GetIndexOfSquare((Row * 10) + col))] = Square()
                                print("yay2")
...
class WeatherEvent:
    ...
    def SetWeatherLocation(self, square_ref):
        self._Location = square_ref
    ...
    def CountDownComplete(self):
        if self._CountDown == 3:
            print("weather event has occured, 2 turns left to move")
            self._CountDown -= 1
            return False
        if self._CountDown == 2:
            self._CountDown -= 1
            print("1 turns left to move")
            return False
        if self._CountDown == 1:
            self._CountDown -= 1
            print("0 turns left to move")
            return False
