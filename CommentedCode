...
class Dastan:
    def __init__(self, R, C, NoOfPieces):
        self._Board = [] #Defines board, players and offer as a list
        self._Players = []
        self._MoveOptionOffer = [] #Move queue is stored in the player class
        self._Players.append(Player("Player One", 1)) #Creates an instance of the player class for player1 and adds it to player list
        self._Players.append(Player("Player Two", -1)) #Creates an instance of the player class for player2 and adds it to the player list
        self.__CreateMoveOptions()#Calls create options function, line 287
        self._NoOfRows = R #6
        self._NoOfColumns = C #6
        self._MoveOptionOfferPosition = 0
        self.__CreateMoveOptionOffer() #Creates the offer, line 198
        self.__CreateBoard() #Line 175
        self.__CreatePieces(NoOfPieces)#No of pieces = 4, line 186
        self._CurrentPlayer = self._Players[0]#Player 1 starts the game
        #After init function is run, the play game function is called(Line 130)
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            if MoveLegal:
                ...
                self._CurrentPlayer.ChangeScore(-(Choice + (2 * (Choice - 1))))#Subtracts from score based on the choice
    ...
    def __CreateBoard(self):
        for Row in range(1, self._NoOfRows + 1):
            for Column in range(1, self._NoOfColumns + 1): #For each coordinate
                if Row == 1 and Column == self._NoOfColumns // 2: #Creates a kotla class for player 1. Kotla class is on line 350
                    S = Kotla(self._Players[0], "K")
                elif Row == self._NoOfRows and Column == self._NoOfColumns // 2 + 1: #Creates a kotla for player 2
                    S = Kotla(self._Players[1], "k")
                else:
                    S = Square() #Creates an instance of the square class, line 318
                self._Board.append(S) # Board is a list instantiated with the dastan class, adds s, a kotla or sqaure to the board list

    def __CreatePieces(self, NoOfPieces):
        for Count in range(1, NoOfPieces + 1): #Creates 4 instances of the piece class for player 1. Piece class is on line 299
            CurrentPiece = Piece("piece", self._Players[0], 1, "!") 
            self._Board[self.__GetIndexOfSquare(2 * 10 + Count + 1)].SetPiece(CurrentPiece)#Get index function is on line 57, set piece is on 325 and sets the cooridnates of the piece
        CurrentPiece = Piece("mirza", self._Players[0], 5, "1") #Creates an instance of the piece cladd for player 1's mirza
        self._Board[self.__GetIndexOfSquare(10 + self._NoOfColumns // 2)].SetPiece(CurrentPiece)
        for Count in range(1, NoOfPieces + 1):#Equivalent code for player 2
    ...
    def __CreateMoveOptionOffer(self):#Adds each move to the offer queue
    ...
    def __CreateRyottMoveOption(self, Direction):#Create an instance of the moveoption class for each possible move option
        NewMoveOption = MoveOption("ryott") #Move option class is on line 370
        NewMove = Move(0, 1 * Direction) #Creates an instance of the move class, line 391
        NewMoveOption.AddToPossibleMoves(NewMove) #Calls function in move class
        NewMove = Move(0, -1 * Direction)
        NewMoveOption.AddToPossibleMoves(NewMove)#X and Y are swaped so coords are in order(Y,X)
        NewMove = Move(1 * Direction, 0)
        NewMoveOption.AddToPossibleMoves(NewMove)
        NewMove = Move(-1 * Direction, 0)
        NewMoveOption.AddToPossibleMoves(NewMove)
        return NewMoveOption #Returns the object new move option, which contains a full list of all possible moves for this move type e.g. ryott, in terms of x and y movement.
    ...
    def __CreateMoveOption(self, Name, Direction):#Direction depends on the player as the players move in separate directions
        if Name == "chowkidar":
    ...
    def __CreateMoveOptions(self): #Adds each possible move to a queue for each player, add to move option queue function is on line 446 in class player
        self._Players[0].AddToMoveOptionQueue(self.__CreateMoveOption("ryott", 1)) #Function create move option is above
...
class Piece:
    def __init__(self, T, B, P, S):
        self._TypeOfPiece = T #normal piece or mirza
        self._BelongsTo = B #Player the piece belongs to
        self._PointsIfCaptured = P
        self._Symbol = S #Symbol that represents the piece on the board
...
class Square: #Class for each empty(non kotla) square
    def __init__(self):
        self._PieceInSquare = None #These will change depending on what piece is in the sqaure
...
class Kotla(Square):#Class for the kotla squares, subclass of square.
    def __init__(self, P, S):
        super(Kotla, self).__init__()
        self._BelongsTo = P #Sets what player owns this kotla
        self._Symbol = S #Sets the symbol for the kotla e.g. K or k
...
class MoveOption:
    def __init__(self, N):
        self._Name = N #Defines this as the name of the move
        self._PossibleMoves = []

    def AddToPossibleMoves(self, M):
        self._PossibleMoves.append(M)#Appends the possible move to the list created above
...
class Move:
    def __init__(self, R, C):
        self._RowChange = R #Define the x and y change of a possible move
...
class Player:
    ...
    def AddToMoveOptionQueue(self, NewMoveOption):
        self.__Queue.Add(NewMoveOption) #Adds the necessary move option to the queue
...
def Main():
    ThisGame = Dastan(6, 6, 4)#Creates instance of dastan class, calls init function
    ThisGame.PlayGame()#Calls play game method of Dastan class, once the dastan class has been instantiated
    print("Goodbye!")
    input()
