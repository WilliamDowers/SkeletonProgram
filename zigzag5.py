    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                Choice = int(input("Choose move option to use from queue (1 to 3) or 9 to take the offer or choose 8 to view the other persons queue: ")) ## wee edit ##
                if Choice == 9:
                    self.__UseMoveOptionOffer()
                    self.__DisplayState()
                if Choice == 8:  ##EDIT##
                    if self._CurrentPlayer.SameAs(self._Players[0]) == True:
                        other_queue = self._Players[1].GetJustQueue()
                        print(other_queue)
                    else:
                        other_queue = self._Players[0].GetJustQueue()
                        print(other_queue)
                    self._CurrentPlayer.ChangeScore(-5)
                    self.__DisplayState()##END EDIT##
...
class Player:
    ...
    def SameAs(self, APlayer):
        if APlayer is None:
            return False     # checks if the player is the same as the other player
