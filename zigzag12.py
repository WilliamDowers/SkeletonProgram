...
class Dastan:
    def __init__(self, R, C, NoOfPieces):
        ...
        self._NoOfPieces = NoOfPieces ## edit##
    ...
    def __CountNormalPieces(self): ## edit ## 
        count = 0
        for S in self._Board:
            PieceInSquare = S.GetPieceInSquare()
            if PieceInSquare is not None:
                
                if PieceInSquare.GetTypeOfPiece() == "mirza":
                    pass
                elif PieceInSquare.GetBelongsTo().SameAs(self._CurrentPlayer):
                    count += 1
                else:
                    pass
    ...
    def __CheckReincarnation(self, FinishSquareReference):##edit##
        row = FinishSquareReference // 10
        index = self.__GetIndexOfSquare(FinishSquareReference)
        normal_pieces = self.__CountNormalPieces()
        possible = False
        if self._CurrentPlayer.SameAs(self._Players[0]):
            if row == 6:
                if normal_pieces < self._NoOfPieces:
                    rein_square = int(input("enter an empty square to reincarnate in your back row"))
                    while self._Board[self.__GetIndexOfSquare(rein_square)].GetPieceInSquare() is not None:
                        #print(self._Board[self.__GetIndexOfSquare(rein_square)].GetPieceInSquare().GetTypeOfPiece())
                        rein_square = int(input("the square must be empty g"))
                    if rein_square // 10 == 1:
                        
                        self._Board[self.__GetIndexOfSquare(rein_square)].SetPiece(Piece("piece", self._Players[0], 1, "!"))
            ...
            if row == 1:
                ...
                if normal_pieces < self._NoOfPieces:
                    rein_square = int(input("enter an empty square to reincarnate in your back row"))
                    while self._Board[self.__GetIndexOfSquare(rein_square)] is not None:
                        rein_square = int(input("the square must be empty g"))
                    if rein_square // 10 == 6:
                        
                        self._Board[self.__GetIndexOfSquare(rein_square)].SetPiece(Piece("piece", self._Players[1], 1, '"'))
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                print(self.__CountNormalPieces())
            ...
            if MoveLegal:
                ...
                self.__CheckReincarnation(FinishSquareReference) ##edit##
