...
class Dastan:
    ...
    def __CalculcateSahmMove(self, StartSquareReference, Direction):
        points = 0## edit ##
        inbounds = True
        while inbounds is True:
            StartSquareReference = StartSquareReference + (10 * Direction)
            index = self.__GetIndexOfSquare(StartSquareReference)
            inbounds = self.__CheckSquareInBounds(StartSquareReference)
            if inbounds == True:
                
                piece = self._Board[index].GetPieceInSquare()
                if self._Board[index].ContainsKotla():
                ...
                if piece is not None:
                    if self._CurrentPlayer.SameAs(piece.GetBelongsTo()):
                        pass
                    else:
                        
                        self._Board[index].RemovePiece()
                        points += piece.GetPointsIfCaptured()
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while not SquareIsValid:
                ...
                sahm = self._CurrentPlayer.ChoiceIsSahm(Choice)
                
                
                if sahm is True:
                    if self._CurrentPlayer.GetSahmStatus():
                        print("you have already used sahm move")
                    else:
                        StartSquareReference = self.__GetSquareReference("containing the square to shoot sahm from")
                        points = self.__CalculcateSahmMove(StartSquareReference, self._CurrentPlayer.GetDirection())
    ...
    def __CreateMoveOptionOffer(self):
        self._MoveOptionOffer.append("sahm")##edit##
    ...
    def __CreateSahmMoveOption(self, Direction):
        NewMoveOption = MoveOption("sahm")## edit ## 
