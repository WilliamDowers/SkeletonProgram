    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                Choice = int(input("Choose move option to use from queue (1 to 3) or 9 to take the offeror 7 to select sacrifice: "))
                if Choice == 9:
                    self.__UseMoveOptionOffer()
                    self.__DisplayState()

                if Choice == 7: ##edit##
                    StartSquareReference = self.__GetSquareReference("containing the piece to sacrifice")
                    piece = self._Board[(self.__GetIndexOfSquare(StartSquareReference))].GetPieceInSquare()
                    #print(self._CurrentPlayer.SameAs(piece.GetBelongsTo()))
                    while piece is None or not self._CurrentPlayer.SameAs(piece.GetBelongsTo()):
                        StartSquareReference = self.__GetSquareReference("containing the piece to sacrifice. make sure it is ur own piece")
                        piece = self._Board[(self.__GetIndexOfSquare(StartSquareReference))].GetPieceInSquare()
                        #print(StartSquareReference)
                    ...
                    self._Board[(self.__GetIndexOfSquare(StartSquareReference))] = Kotla(self._CurrentPlayer, kolta)
